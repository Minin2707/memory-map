...
void main() {
  group('buildStoryMemoriesYearSections', () {
    test('shouldReturnEmptySectionsForEmptyInput', () {
      expect(buildStoryMemoriesYearSections(const <MemoryReadModel>[]), isEmpty);
    });

    test('shouldGroupMemoriesByYearWithNewestYearFirst', () {
      final sections = buildStoryMemoriesYearSections(<MemoryReadModel>[
    ...
    test('shouldNotMutateSourceList', () {
      ...
      buildStoryMemoriesYearSections(source);

      expect(source, <MemoryReadModel>[second, first]);
    });

    test('shouldExposeYearCountsAndPreservePreviewMetadata', () {
      ...
      final sections = buildStoryMemoriesYearSections(<MemoryReadModel>[
        source,
        readModel(memory(id: 'memory-second', year: 2024)),
      ]);

      expect(sections.single.memoryCount, 2);
      expect(sections.single.memories.first.previewPhoto, same(preview));
    });
  });

  group('StoryMemoriesYearSection', () {
    test('shouldExposeUnmodifiableMemoriesAndSafeDiagnostics', () {
      final section = StoryMemoriesYearSection(
        year: 2024,
